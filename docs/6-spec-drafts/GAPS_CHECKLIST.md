...
| T1-03 | **Combat Modifier Pipeline & Proc Order** — Reframed: 11-step expansion of `apply_combat_math` with explicit proc trigger points (on-hit, on-crit, on-kill, on-block, reactive). Sequential evaluation, deferred reactive procs. | `DRAFTING` | Reframed from event ordering to modifier pipeline | [draft](tier-1-combat/03-combat-resolution-ordering.md) | `3-gameplay-systems/01-rpg-mechanics.md` |
