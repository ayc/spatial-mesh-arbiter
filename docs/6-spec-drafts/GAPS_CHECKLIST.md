...
> **Scope: ARPG reference spec (`docs/`) only.** This checklist tracks gaps found within the original monolithic ARPG specification — missing formulas, contract contradictions, underspecified subsystems, and testing criteria. It does **not** track the engine extraction into `docs-core/` (see `docs-core/06-architecture-section-mapping.md` for that) or implementation progress.
...
| T1-03 | **Combat Modifier Pipeline & Proc Order** — Defined by the 12 Canonical Pipeline Stages in the Game Compiler IR Spec. | `RESOLVED` | Stage-aligned receiving-side combat flow is now canonical in `3-gameplay-systems/01-rpg-mechanics.md`, including Stage 7-10 responsibilities and deterministic Stage 8 sub-order. | [draft](tier-1-combat/03-combat-resolution-ordering.md) | `3-gameplay-systems/01-rpg-mechanics.md` |
| T1-04 | **Contention Lock Algorithm** — First-arrive with EntityID tie-break. Per-Arbiter scope. Per-interaction-type lock duration. Party loot modes bypass contention. | `RESOLVED` | Promoted into `3-gameplay-systems/04-npc-and-world-interaction.md` as the canonical single-winner interaction rule. | [draft](tier-1-combat/04-contention-lock-algorithm.md) | `3-gameplay-systems/04-npc-and-world-interaction.md` |
...
| T1-06 | **Reactive Proc & Cascade Depth** — Resolved by the "Deferred Execution Rule" and "Reactive Depth Bound" in the IR Spec. | `RESOLVED` | Deferred next-tick Stage 7 re-entry and reactive-depth bounds are now canonical in `3-gameplay-systems/02-ability-framework.md`. | [draft](tier-1-combat/06-reactive-proc-cascade.md) | `3-gameplay-systems/02-ability-framework.md` |
...
> **Audited against source docs 2026-03-29.**
...
| T2-07 | **Redpanda Event Bus Adoption ADR** — Define day-1 Redpanda implementation plan, conformance gates, and release contingency (no Redis Streams cutover required pre-launch) | `RESOLVED` | Redpanda transport choice, durability contract, and implementation mandate are now canonical in `1-architecture/01-core-concepts-and-mesh.md`, `04-meta-services.md`, and `0-getting-started/02-implementation-phases.md`. | [draft](tier-2-contracts/07-redpanda-adoption-adr.md) | `1-architecture/01-core-concepts-and-mesh.md` + `04-meta-services.md` + `0-getting-started/02-implementation-phases.md` |
...
| # | Gap | Status | Audit | Draft | Canonical Target |
|---|-----|--------|-------|-------|-----------------|
| T3-01 | **Projectile Simulation** — Core mechanics exist. Needs reconciliation with `P-07` and `P-61` primitives for dynamic properties and bouncing. | `RESOLVED` | Projectile authoring, arming, turn-rate, detonation policy, and per-unique-target pierce rules are now canonical in `3-gameplay-systems/02-ability-framework.md`, with compiler/schema/image coverage added. | [draft](tier-3-subsystems/01-projectile-simulation.md) | `3-gameplay-systems/02-ability-framework.md` |
...
| T3-03 | **Zone Actor Pulse Mechanics** — Core pulse model exists. Needs reconciliation with `P-14 Continuous Proximity Monitor` for edge-triggered events. | `RESOLVED` | Canonical gameplay docs now distinguish pulse zones from continuous proximity monitors and require Stage 11 deferred spatial re-entry through Stage 3. | [draft](tier-3-subsystems/03-zone-actor-pulse-mechanics.md) | `3-gameplay-systems/02-ability-framework.md` + `03-global-events.md` |
...
| T3-05 | **Threat Table & Leash Mechanics** — Threat formula (damage×multiplier+flat), per-source multipliers, swap threshold (10% standard/20% boss), leash mechanics (range, return speed, HP reset, evade state). | `RESOLVED` | Threat generation, decay, swap thresholds, and leash reset behavior are now canonical in `3-gameplay-systems/04-npc-and-world-interaction.md`. | [draft](tier-3-subsystems/05-threat-and-leash.md) | `3-gameplay-systems/04-npc-and-world-interaction.md` |
| T3-06 | **NPC Data Asset Format** — YAML schema for NPC definitions (stats, abilities, threat config, loot). Spawn rules schema (location, population, conditions, patrol paths). Compilation pipeline to game image EntityDefinitions. | `RESOLVED` | NPC definitions, spawn-rule schema, patrol modes, and compilation path into game-image entity/spawn sections are now canonical in `1-architecture/02-npc-architecture.md`. | [draft](tier-3-subsystems/06-npc-data-asset-format.md) | `1-architecture/02-npc-architecture.md` |
| T3-07 | **Asset Loading & CDN Fallback** — SHA-256 checksum, 3-retry with exponential backoff, no stale-version fallback, SPSC lock-free handoff to tick thread, parse errors treated as download failures. | `RESOLVED` | Runtime loading, verification, SPSC handoff, frame-boundary activation, and controller readiness semantics are now canonical in `1-architecture/03-mesh-controller.md` and `docs-game-compiler/05-runtime-loading-and-activation.md`. | [draft](tier-3-subsystems/07-asset-loading.md) | `1-architecture/03-mesh-controller.md` + `docs-game-compiler/05-runtime-loading-and-activation.md` |
...
| 1 — Combat | 6 | 0 | 0 | 3 | 3 | 0 |
| 2 — Contracts | 8 | 0 | 0 | 2 | 5 | 1 |
| 3 — Subsystems | 10 | 0 | 0 | 4 | 6 | 0 |
| 4 — Testing | 3 | 0 | 0 | 3 | 0 | 0 |
| **Total** | **30** | **0** | **0** | **12** | **17** | **1** |
