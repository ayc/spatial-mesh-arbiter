...
| `keyframe_interval` | `10` ticks | Update frequency for entities in the outer visible ring (166ms). |
...
| Variable | Recommended Default | Description |
| :--- | :--- | :--- |
| `safe_threshold` | `300` entities | Density below which the server runs at 1.0 scale. |
| `critical_threshold`| `1000` entities | Density at which maximum physical slowdown is applied. |
| `min_dilation` | `0.2` (1/5th speed)| Absolute floor for simulation speed. |
| `curve_exponent` | `2.0` (Quadratic) | Shape of the slowdown ramp-up. |

### 1.3 Gameplay Lifecycle
| Variable | Recommended Default | Description |
| :--- | :--- | :--- |
| `resurrect_window` | `600` ticks | 10-second window for healers before hard player despawn. |
| `logout_fuse` | `3600` ticks | 60-second vulnerability timer for wilderness logouts. |
| `handover_buffer` | `2` ticks | Lead-time for deterministic RUDP ownership flips. |
...
| Variable | Recommended Default | Description |
| :--- | :--- | :--- |
| `bucket_capacity` | `24` tokens | Maximum burst size for per-entity action proposals. |
| `bucket_refill` | `2` per tick | Actions allowed per second (120/sec). |
...
| Variable | Recommended Default | Description |
| :--- | :--- | :--- |
| `max_event_age` | `60` ticks | **Horizon for Idempotency Ledger.** Packets older than 1s are dropped. |
