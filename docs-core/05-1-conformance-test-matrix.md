...
| `API-01` | Adapter API v2 | `SCN-API-STAGE-ID-CLOSED-SET` | `dispatch_stage` called with invalid `DispatchStageId` values 0, 2, or 13+ MUST be rejected by the adapter with `FAULT` status. Only values in the normative `DispatchStageId` set (1, 3-12) are accepted. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-02` | Adapter API v2 | `SCN-API-STAGE-ORDER-MONOTONIC` | Within a single tick, `dispatch_stage` calls MUST arrive in strictly ascending `DispatchStageId` order. Out-of-order invocation MUST be detectable and emit a fault signal. | `PR`, `NIGHTLY`, `RELEASE` |
...
| `API-06` | Adapter API v2 | `SCN-API-SPAWN-CONFIG-SCHEMA` | `initialize_spawn_configuration` response MUST include a `spawn_configuration` with required fields: `initial_entity_state` (valid `SchemaTypedPayload` whose `(payload_type_id, schema_version)` pair is declared and whose body validates against the declared schema), `ghost_movement_replication_cadence` (valid `GhostMovementReplicationCadence` enum, 0-3), `lifetime_ticks` (positive or null). The response MUST NOT contain `entity_id` (engine-owned, provided in the request). Missing or malformed fields MUST cause a deterministic spawn-time reject/fault and a conformance failure. | `PR`, `NIGHTLY`, `RELEASE` |
...
| `API-09` | Adapter API v2 | `SCN-API-DEFERRED-EVENT-ENVELOPE` | Every entry in `deferred_events` MUST include `target_stage`, `event_class`, `source_entity_id`, `ready_tick`, `sort_key`, and `payload`, and `payload` MUST be a valid `SchemaTypedPayload`. `target_stage` MUST be a valid `DeferredTargetStageId`. Under the current profile, only `deferred_spatial_event -> Stage 3` and `deferred_combat_event -> Stage 7` are valid. Missing fields, unknown classes, invalid class/stage pairs, or invalid schema-typed payloads MUST cause a conformance failure. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-10` | Adapter API v2 | `SCN-API-DEFERRED-EVENT-ORDERING` | Before stage injection, mixed `incoming_deferred_events` batches MUST be sorted deterministically by `ready_tick`, then `target_stage`, then `sort_key`. Replay of the same deferred batch MUST yield identical stage admission order and digest. | `NIGHTLY`, `RELEASE` |
| `API-11` | Adapter API v2 | `SCN-API-SCHEMA-TYPED-PAYLOAD-DECLARED` | Every boundary `SchemaTypedPayload` MUST use a `(payload_type_id, schema_version)` pair declared by `payload_schema_descriptors` during compatibility negotiation. Undeclared type/version pairs MUST be rejected or faulted before mutation. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-12` | Adapter API v2 | `SCN-API-SCHEMA-TYPED-PAYLOAD-VALID` | Every boundary `SchemaTypedPayload.body` MUST validate against the schema identified by its declared `(payload_type_id, schema_version)` pair. Schema-invalid bodies MUST be rejected or faulted before mutation. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-13` | Adapter API v2 | `SCN-API-HOOK-RESPONSE-PAYLOAD-EXCLUSIVE` | Only the typed response payload field appropriate for the invoked hook MAY be populated. Any response that populates more than one of `stage_outcome`, `terminal_outcome`, `spawn_configuration`, or `compatibility_descriptor`, or populates the wrong one for the hook, MUST cause a conformance failure. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-14` | Adapter API v2 | `SCN-API-COMPATIBILITY-DESCRIPTOR-SCHEMA` | `describe_compatibility` MUST return a valid `compatibility_descriptor` containing `adapter_identity`, `adapter_version_semver`, `adapter_api_major`, non-empty `supported_wire_schema_versions`, and well-formed `payload_schema_descriptors`. Missing required fields or malformed descriptors MUST cause a conformance failure. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-15` | Adapter API v2 | `SCN-API-STAGE-OUTCOME-PAYLOAD-DISCRIMINATOR` | `MutationRecord` and `ImmediateEvent` payloads in `stage_outcome` MUST use `payload.payload_type_id` as the normative type discriminator. Missing `payload_type_id` / `schema_version`, undeclared payload types, or parallel open-string classifiers MUST cause a conformance failure. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-16` | Adapter API v2 | `SCN-API-DEFERRED-SORT-KEY-STABILITY` | For identical stage-local inputs under replay, the adapter MUST emit identical `sort_key` values for deferred events. If one stage invocation emits multiple deferred events for the same `ready_tick` and `target_stage`, the emitted `sort_key` set MUST encode a deterministic total order. | `NIGHTLY`, `RELEASE` |
| `API-17` | Adapter API v2 | `SCN-API-DEFERRED-SPATIAL-STAGE3-REENTRY` | Stage 11 timer work whose payload requires a spatial query (for example projectile pulses or zone actors) MUST emit `deferred_spatial_event` targeting Stage 3 (`TargetResolution`). Emitting such work as `deferred_combat_event`, targeting Stage 7, or resolving it immediately is non-conformant. | `PR`, `NIGHTLY`, `RELEASE` |
| `API-18` | Adapter API v2 | `SCN-API-SPAWN-CONFIG-ARCHETYPE-PROJECTILE` | `initialize_spawn_configuration` for a projectile/trap archetype MUST return `initial_entity_state` whose projectile fields match the compiled archetype configuration in the game image, including `arming_delay_ticks`, `homing`, `turn_rate`, `pierce`, and detonation-policy fields. Replay of the same spawn request MUST yield identical projectile config bytes/digest. | `NIGHTLY`, `RELEASE` |
| `API-19` | Adapter API v2 | `SCN-API-SPAWN-CONFIG-NPC-ENTITYDEF` | `initialize_spawn_configuration` for an NPC archetype MUST return `initial_entity_state` consistent with the compiled entity definition, including authored ability references, threat/leash parameters, lifecycle values, and runtime-tier metadata. Missing or divergent compiled fields are non-conformant. | `NIGHTLY`, `RELEASE` |
